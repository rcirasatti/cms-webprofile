...
const HeroSection = ({ handleExploreClick, content = [], preview = false }) => {
...
return (
    <section
    id="home"
    className={`relative ${preview ? 'h-96' : 'min-h-screen'} flex items-center justify-center overflow-hidden bg-gradient-to-br from-[hsl(210,72%,25%)] via-[hsl(148,41%,58%)]/20 to-[hsl(146,51%,91%)] ${preview ? '' : 'pt-16'}`}
    >
    ...
    <div className="container mx-auto px-4 text-center relative z-10">
        <div className={`space-y-${preview ? '6' : '8'} animate-fade-in`}>
        <div className="relative inline-block">
            <h1 className={`${preview ? 'text-3xl md:text-5xl' : 'text-5xl md:text-7xl'} font-bold tracking-tight text-white drop-shadow-lg`}>
        ...
        <p className={`${preview ? 'text-lg md:text-xl' : 'text-xl md:text-2xl'} font-medium mb-8 max-w-3xl mx-auto text-white/90 animate-fade-in-delay-1 leading-relaxed drop-shadow-md`}>
            {subtitle}
        </p>

        <div className={`flex flex-col items-center space-y-${preview ? '6' : '8'} animate-fade-in-delay-2`}>
            <button
            onClick={handleExploreClick}
            className={`group relative ${preview ? 'px-6 py-3' : 'px-8 py-4'} bg-gradient-to-r from-[hsl(148,41%,58%)] via-[hsl(210,72%,25%)] to-[hsl(148,41%,58%)] rounded-xl font-medium text-white transition-all duration-500 hover:scale-105 hover:shadow-[0_0_30px_hsl(148,41%,58%,0.4)] active:scale-95 overflow-hidden backdrop-blur-sm`}
            >
